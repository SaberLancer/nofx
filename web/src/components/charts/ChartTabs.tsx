...
export function ChartTabs({ traderId, selectedSymbol, updateKey, exchangeId, candidateSymbols = [] }: ChartTabsProps) {
  ...
  const candidateSymbolOptions = useMemo(
    () =>
      candidateSymbols
  ...
  const filteredCandidateSymbols = candidateSymbolOptions.filter((s) =>
    s.toLowerCase().includes(searchFilter.toLowerCase())
  )

  // For crypto dashboard use candidate symbols dropdown (no manual query needed)
  useEffect(() => {
    if (marketType !== 'crypto' || candidateSymbolOptions.length === 0) return
    if (!candidateSymbolOptions.includes(chartSymbol)) {
      setChartSymbol(candidateSymbolOptions[0])
    }
  }, [marketType, candidateSymbolOptions, chartSymbol])
        ...
        {activeTab === 'kline' && (
          <div className="flex items-center gap-2 md:gap-3 w-full md:w-auto min-w-0">
            ...
            <div className="shrink-0 relative" ref={dropdownRef}>
              {marketConfig.hasDropdown || marketType === 'crypto' ? (
                <>
                  <button
                      ...
                      {marketType === 'crypto' ? (
                        <>
                          ...
                          <div className="overflow-y-auto max-h-60 custom-scrollbar">
                            {filteredCandidateSymbols.length > 0 ? (
                              filteredCandidateSymbols.map((sym) => (
                                <button
                                  key={sym}
                                  onClick={() => { setChartSymbol(sym); setShowDropdown(false); setSearchFilter('') }}
                                  className={`w-full px-3 py-2 text-left text-[11px] font-mono hover:bg-white/5 transition-all ${chartSymbol === sym ? 'bg-nofx-gold/10 text-nofx-gold' : 'text-nofx-text-muted'}`}
                                >
                                  {sym}
                                </button>
                              ))
                            ) : (
                              <div className="px-3 py-3 text-[11px] text-nofx-text-muted">
            ...
            {/* Manual symbol input removed: use candidate dropdown for switching */}
          </div>
        )}
      </div>
