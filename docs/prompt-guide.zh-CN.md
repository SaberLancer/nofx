...
### 持仓盈利率：代码自动平仓（与 Prompt 无关也会执行）

策略风控中的 **Margin 盈亏%** 规则由后端在每轮**扫描周期**拉完持仓后检查，并在**仪表盘拉账户**时复检（无独立「每分钟」任务）。网格策略不启用。
...
「盈利较高且出现反转再平仓」等仅写在提示词里，**代码不会**因此自动下单。详见 [architecture/POSITION_PNL_ENFORCEMENT.zh-CN.md](architecture/POSITION_PNL_ENFORCEMENT.zh-CN.md)。
